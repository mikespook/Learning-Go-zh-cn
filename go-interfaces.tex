...
Recall the Bubblesort exercise (\ref{ex:bubble}), where we sorted an
array of integers:
\begin{lstlisting}
...
\end{lstlisting}
This fails with
\error{cannot use n (type []int) as type []interface { } in function argument}

This is because Go will implicitly convert a single \type{string} or
\type{int} (or whatever) to an interface, this is very easy to
do. Go just puts the type inside an interface. This costs $O(1)$. With a slice Go needs to copy (and convert) each element to a
\emph{new} slice.  This means the new slice now refers to a different
underlaying array and this can potentially take a long time, at least
$O(n)$, hence Go does \emph{not} do this.
For those interested the full thread can be found at
\cite{go_nuts_interfaces}.

So what is the Go way of creating such a "generic" function? 
But instead of doing the type inference our selves, we handover 
...
We will at least a function to get the length of the array,
...
\end{lstlisting}
\item Define new types for the arrays we want to sort;
\begin{lstlisting}
type Xi []int
type Xs []string
\end{lstlisting}
\item Implement the functions of the \type{S} interface.
For integers:
\begin{lstlisting}
...
strings := Xs{"aap", "noot", "mies", "miek", "Go"}
