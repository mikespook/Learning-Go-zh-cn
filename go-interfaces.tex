...
to discover the type at \emph{run-time}. See section "\titleref{sec:introspection}" for more.
...
create a pointer to an interface value. The release notes for that release \gorelease{2010-10-13}
that
...
by using a \first{\emph{type assertion}}{type assertion}.
...
Lastly you can use a "comma, ok" form to see if an interface type implements
...
\end{lstlisting}
you are sure a variable implements an interface you can use:
\begin{lstlisting}
t := any.(I)
\end{lstlisting}
...
func show(i interface{}) {
    switch t := i.(type) {
      case *Person:
        r := reflect.ValueOf(i) |\coderemark{Enter the world of reflection}|
...
going for the type. Thus we need\newline\lstinline{Elem().Type().(*reflect.StructType)} to get to it;}|
	  r.(*reflect.Ptr).Elem().Type().(*reflect.StructType).Field(0).tag
...
Going from a \type{*Person} via \mbox{\type{*reflect.Ptr}} using the %
...
on a \type{Value} in the reflection world.}|
	  r.(*reflect.Ptr).Elem().(*reflect.StructValue).Field(0).|\newline|(*reflect.StringValue).Get()
    }
}
\end{lstlisting}
\showremarks
