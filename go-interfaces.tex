...
\todo{And why is this exactly?}

So what is the Go way of creating such a "generic" function? Interfaces
of course. Instead of doing the type inference our selves, we handover 
that task to Go.
The following steps are required:
\begin{enumerate}
\item Define an interface type (S) with a number of methods needed for
sorting.
...
\end{lstlisting}
\item Define new types for the arrays you want to sort;
\begin{lstlisting}
type Xi []int
type Xs []string
\end{lstlisting}
\item Implement the functions of the Sort interface.
For integers:
\begin{lstlisting}
...
\end{lstlisting}
\item Write a generic sort function that works on the S interface.
\begin{lstlisting}
...
%%	strings := StringArray{"kobe", "odom", "farmar", "fisher", "artest", "bynum", "brown", "gas
%%
%%	Sort(numbers)
%%	fmt.Printf("%v\n", numbers)
%%
