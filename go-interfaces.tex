...
\todo{Tell how to check if a type implements an interface}
...
create a pointer to an interface value. The release notes leave no room
for doubt:
\begin{qote}
...
With interfaces you define wat types much a certain interface and interface itself
are just a definition. The implementation of that definition is done with types
and methods on those types. Putting methods on definitions would lead to mixing
the two.
