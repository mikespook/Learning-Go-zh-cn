...
\begin{figure}[h]
\caption{Pointers and types, the values \var{v} all have type \type{X}}
\label{fig:pointers}
\begin{center}
...
\first{\key{new}}{keyword!new} and \first{\key{make}}{keyword!make}. 
...
allocates an array of 100 \type{int}s and then creates a slice structure with
...
\begin{itemize}
\item{
From a \lstinline{string} to a slice of \lstinline{byte}s or
\lstinline{int}s.
\begin{lstlisting}
...
\item{
From a slice of \lstinline{byte}s or \lstinline{int}s to a \lstinline{string}.
\begin{lstlisting}
...
\todo{"reference-y" sounds like a pretty hazy concept to me. As has been
noted already, struct types with non-exported fields cannot be copied
by code outside of the type's package. This forces the type's users to
...
\end{lstlisting}
This is (you might have guessed) a \first{\emph{function call}}{function
call}.
