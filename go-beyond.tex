...
on a \type{Value} in the reflection world: We can explain the code
at \gocircle{2} in a figure as follows:
\begin{figure}[H]
\caption[Pealing away the layers using reflection]{Pealing away the layers using reflection. Going
from a \type{Person} via \mbox{\type{*reflect.Value}} using the methods described in \prog{godoc reflect} to get the actual \type{string} contained deep within.}
