...
\todo{From Rob Pike
Appendix B is misleading in its treatment of Go.  The only Go
expressions that can be statements are function calls, method calls,
and channel operations.  It's almost like Fortran. The text implies
that assignment is an expression, which is not true.  The confusion
about = in if conditions is simply not present in Go.
