...
\prog{helloworld}:
\lstinputlisting[language=make,caption=Makefile for a
...
is always represented in 32 bits (even on 64 bit architectures).
...
\item \key{package} and \key{import} are briefly touch upon in section "\titleref{sec:hello world}". 
...
\item \key{select}, see chapter \ref{chap:channels};
\item \key{struct}, see chapter \ref{chap:beyond};
