...
\subsection{Slices}
\label{sec:slices}
A slice is simular to an array, but it can grow when new elements
are added.
...
From the language specificiation \cite{go_spec}:
\begin{quote}
...
copied is the minimum of \lstinline{len(src)} and \lstinline{len(dst)}.
\end{quote}
\index{built-in!copy}
\begin{lstlisting}
