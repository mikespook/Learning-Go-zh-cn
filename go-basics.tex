...
参阅 "\titleref{sec:arrays}" 章节了解更多关于 slice、
数组和函数 \func{cap} 的详细信息。
\index{built-in!len}\index{built-in!cap}

\paragraph{\func{new}} 用于各种类型的内存分配。
参阅 "\titleref{sec:allocation with new}" 在 \pageref{sec:allocation with new}。
\index{built-in!new}

\paragraph{\func{make}} 用于内建类型（map、slice 和 channel）的内存分配。
参阅 "\titleref{sec:allocation with make}" 在 \pageref{sec:allocation with make}。
\index{built-in!make}
...
参阅 "\titleref{sec:panic}" 在 \pageref{sec:panic} 了解更多信息。
...
\first{complex numbers}{complex numbers}。有了之前给的简单的例子，不用再进一步讨论复数了。
...
如果不使用零来初始化它，则用\first{composite
literal}{composite literal}：\lstinline|a := [3]int{1, 2, 3}| 也可以简写为
...
从 \gorelease{2010-10-27} 这个语法使用更加简单了。
来自于发布记录：
\begin{quote}
...
这是其与 array 不同的地方；
slice 是\first{reference types}{reference types}，
...
slice总是与一个固定长度的 array 成对出现。对 slice 定义了
\first{capacity}{slice!capacity} 和 \first{length}{slice!length}。
...
a := [...]int{1, 2, 3, 4, 5} |\longremark{定义一个 5 个元素的 array，
序号从 0 到 4；}|
...
在 \ref{src:arrays} 列出的代码中，我们尝试在第八行做一些不可能的处理，
