...
\label{sec:constants}
\todo{typed constants, see crypto/rsa for instance}
...
You may also explicitly type a constant:
\begin{lstlisting}
const (
	a int = 0
	b string = "B"
...
would be to use \index{string literals} by using back-quotes: \key{`}:
\begin{lstlisting}
s := `Starting part
     Ending part`
\end{lstlisting}
...
zero to index 3, this is thus short for: \texttt{a[0:4]}, and yields: \texttt{1, 2, %
3, 4};}|
