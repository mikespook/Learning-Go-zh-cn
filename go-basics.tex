...
\subsection{If}
In Go a simple \first{\key{if}} looks like this:
\begin{lstlisting}
...
\subsection{Goto}
Go has a \first{\key{goto}} statement --- use it wisely. With \key{goto}
...
\subsection{For}
The Go \first{\key{for}} loop has three forms, only one of
...
\subsection{Break}
With \first{\key{break}} you can quit loops early, \key{break} breaks
the current loop.
\begin{lstlisting}
for i := 0; i < 10; i++ {
    println(i)
...
\subsection{Switch}
Go's \first{\key{switch}} is more general than C's. The expressions need
not be
