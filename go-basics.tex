\epi{"There and back again."}{\textit{Lord of the Rings}\\\textsc{Bilbo Baggins}}
...
\first{goroutines}\footnote{Yes, that sounds a lot like
...
Go supports both the \texttt{\rem{/* */}} and \texttt{\rem{//}} types of
comments. There is no need to put a semicolon at the end of each line
(as you would in C).
...
%%\subsection{Some words about syntax}
%%\todo{syntax} ??
%% Something about syntax here
%%func g()
%%{                  // INVALID
%%}
...
measured in mere seconds, even for large programs.
\begin{display}
\pr 6g helloworld.go		  \qquad\qquad\qquad\coderemark{compiles to helloworld.6 (for 64 bit)}
\pr 6l -o helloworld helloworld.6 \coderemark{linking stage}
\end{display}
...
\subsection{Building a program}
\label{sec:building a program}
Another, less laborious, way to build a Go program, is to use a
...
\type{int} variables. You can even make use of parallel assignment:
\begin{lstlisting}
a, b := 20, 16
\end{lstlisting}
...
\first{Keywords!\key{iota}}\footnote{The word [iota] is used in a common English phrase,
...
\end{lstlisting}
You can even do the following. The first use of \key{iota} will yield 0, so \var{a} is equal to 0, whenever
...
\begin{lstlisting}
s := "hello"
c := []byte(s)	    |\longremark{Convert \texttt{s} to an array of \type{byte}s, see %
...
\paragraph{\func{panic} and \func{panicln}} used for an 
...
There are only a few control structure in Go.
...
\subsection{If}
In Go a simple \first{Keywords!\key{if}} looks like this:
\begin{lstlisting}
...
\subsection{Goto}
Go has a \first{Keywords!\key{goto}} statement --- use it wisely. With \key{goto}
...
\subsection{For}
The Go \first{Keywords!\key{for}} loop has three forms, only one of
...
expressions (see appendix \ref{chap:expression}), if you want to run multiple variables in a \key{for} you should
...
\subsection{Break}
With \first{Keywords!\key{break}} you can quit loops early, \key{break} breaks
the current loop.
\begin{lstlisting}
for i := 0; i < 10; i++ {
    println(i)
...
\subsection{Switch}
Go's \first{Keywords!\key{switch}} is more general than C's. The expressions need
not be
...
don't mix the two.
\subsection{Slices}
A slice refers to an under laying array. What makes slices different than
...
\subsection{Maps}
\label{sec:maps}
A lot of other languages have a similar type built-in, Perl has hashes
...
\end{lstlisting}
Adding elements to the \type{map} would be done as:
\begin{lstlisting}
...
\end{lstlisting}
And finally you can remove elements from the \type{map}:
\begin{lstlisting}
