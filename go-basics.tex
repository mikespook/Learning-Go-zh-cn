...
assigned to it, is discarded. In this example we only assign a value
to \var{b}.
\begin{lstlisting}
_, b := 34, 35;
\end{lstlisting}
...
a channel, a \key{range} clause can manage the loop for you. The
\key{range} function(?) returns two values, the index or key and the 
corresponding value.
\begin{lstlisting}
