...
\first{\key{iota}}{Keywords!iota}\footnote{The word [iota] is used in a common English phrase,
...
\first{complex numbers}{complex numbers}.
...
\subsection{If}
In Go a simple \first{\key{if}}{Keywords!if} looks like this:
\begin{lstlisting}
...
\subsection{Goto}
Go has a \first{\key{goto}}{Keywords!goto} statement --- use it wisely. With \key{goto}
...
\subsection{For}
The Go \first{\key{for}}{Keywords!for} loop has three forms, only one of
...
expressions (see appendix \ref{chap:articles}), if you want to run multiple variables in a \key{for} you should
...
\subsection{Break}
With \first{\key{break}}{Keywords!break} you can quit loops early, \key{break} breaks
the current loop.
\begin{lstlisting}
for i := 0; i < 10; i++ {
    println(i)
...
a fixed size. For slices we define a \first{capacity}{capacity} and a
\first{length}{length}.
