...
\first{channel}{channel} \cite{csp, hoare} 完成；
...
连接器用相同的命名方式：\verb|<数字>l|。go 的工具链被抽象并隐藏在叫做 \prog{go}\index{tooling!go!tool} 
的工具后面。不过仍然可以直接访问编译器和链接器。

为了编译上面的代码，执行：
\begin{display}
\pr \user{go tool 6g helloworld.go}
\end{display}
\index{tooling!go!6g}

\noindent{}并且用 \prog{go tool 6l} 链接它：

\begin{display}
\pr \user{go tool 6l helloworld.6}
\end{display}
\index{tooling!go!6l}

\noindent{}然后执行：
\begin{display}
\pr \user{./6.out}	    \coderemark{默认的（64位） Go 可执行文件名（32 位上是 8.out）}
\end{display}
\vspace{-3.0ex}
...
另一种可以减少麻烦且是首选的编译 Go 程序的办法，是使用工具
\prog{go}。\index{tooling!go}
工具 \prog{go} 使用指向一个目录的配置参数 \var{\$GOPATH}：
\begin{display}
\pr mkdir -p ~/g/src    # 建立目录 ~/g 和 ~/g/src
\pr GOPATH=~/g          # 设置 GOPATH 为 ~/g
\end{display}

建立一个存放 \prog{helloworld} 的目录：
\begin{display}
\pr mkdir ~/g/src/hello             # 在 src 中建立 hello 子目录
\pr cp helloworld.go ~/g/src/hello  # 复制源码
\end{display}
...
这个结果是叫做 \prog{a.out} 的可执行文件。如果要用指定的名字，可以使用输出参数（-o）：
\begin{display}
\pr go build -o helloworld
\end{display}
...
\subsection{错误}
任何足够大的程序或多或少都会需要使用到错误。因此 Go 有为了错误而存在的内建类型，叫做 \lstinline{error}。
...
\item \key{go} 用于并行（第 \ref{chap:channels} 章）；
...
\paragraph{\func{close}} 用于 channel
通讯。使用它来关闭 channel，参阅第 \ref{chap:channels} 章了解更多。
\index{built-in!close}

\paragraph{\func{delete}} 用于在 map 中删除实例。
\index{built-in!delete}

\paragraph{\func{len} 和 \func{cap}} 可用于不同的类型，
...
\paragraph{\func{new}} 用于各种类型的内存分配。
参阅 "\titleref{sec:allocation with new}" 在 \pageref{sec:allocation with new} 页。
\index{built-in!new}

\paragraph{\func{make}} 用于内建类型（map、slice 和 channel）的内存分配。
参阅 "\titleref{sec:allocation with make}" 在 \pageref{sec:allocation with make} 页。
\index{built-in!make}

\paragraph{\func{copy}} 用于复制 slice。\func{append} 用于追加 slice。
参阅本章的 "\titleref{sec:slices}"。
\index{built-in!copy}
\index{built-in!append}

\paragraph{\func{panic} 和 \func{recover}} 用于\emph{异常}处理机制。
参阅 "\titleref{sec:panic}" 在 \pageref{sec:panic} 页了解更多信息。
\index{built-in!panic}
\index{built-in!recover}

\paragraph{\func{print} 和 \func{println}} 是底层打印函数，可以在不引入
\package{fmt}\index{package!fmt} 包的情况下使用。它们主要用于调试。
\index{built-in!print}\index{built-in!println}

\paragraph{\func{complex}、\func{real} 和 \func{imag}} 全部用于处理
...
参阅第 \pageref{sec:constructors and composite literals} 页的"\titleref{sec:constructors and composite literals}"章节了解更多信息。}
...
(\verb|...|)。%
\gomarginpar{Go 发布版 \gorelease{2010-10-27} \cite{go_release_hist}。}
从发布版 \gorelease{2010-10-27}\cite{go_release_hist} 开始这个语法就变得更加简单了。
来自于发布记录：
\begin{quote}
...
的方式创建。
这会创建一个新的 slice，指向 \lstinline{a}，从序号 \var{I} 开始，结束在序号 \var{J}之前。
长度为 \lstinline{J - I}。
