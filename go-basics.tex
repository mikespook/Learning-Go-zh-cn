...
\lstinputlisting[language=make,caption=程序的 Makefile,numbers=right,linerange={5,11}]{src/Makefile.prog}
...
Go 程序的构建系统在将来可能会发生变化，从而 \prog{make} 会被移除。所以我们使用 \prog{gomake} 进行构建。
...
而 C++ 同样也有 map（在 \package{lib}\index{package!lib}中）。 
