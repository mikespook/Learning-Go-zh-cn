...
all the compiles.
\begin{display}
\pr 6g helloworld.go		 \coderemark{compiles to helloworld.6 (for 64 bit)}
\end{display}
...
\begin{lstlisting}
var s string = "hello"
s[0] = 'c'  |\coderemark{Change to 'c', this is an error}|
\end{lstlisting}
...
The \first{\key{range}}{keyword!range} keyword can also be used on
arrays, slices, maps (see section "\titleref{sec:arrays}") and channels
(see chapter \ref{chap:channels}, section ??).
If used \key{range} is also an
