...
And then we run it:
\begin{display}
\pr ./6.out	    \coderemark{The default name for a (64 bit) Go executable}
\end{display}
\vspace{-3.0ex}
...
\lstinline{float32} and \lstinline{float64}. A 64 bit integer or
floating point value is \emph{always} 64 bit, also on 32 bit
architectures.
...
s := "Starting part" +
     "Ending part"
\end{lstlisting}
Then Go will not insert the semicolons in the wrong places.
\end{lbar}
\subsection{Complex numbers}
Go has native support for complex numbers. If you 
use them you need a variable of the type \lstinline{complex}. If
you need to specify the number of bits you have \lstinline{complex64} and
\lstinline{complex128} for 32 and 64 bits real and imaginary parts. 
...
\first{\key{bit wise and}}{operator!bit wise and}, 
\first{\key{bit wise or}}{operator!bit wise or}, \first{\key{bit wise xor}}{operator!bit
wise xor} and \first{\key{bit clear}}{operator!bit wise clear} respectively.
The \verb|&&|, \verb/||/ and \verb|!| operators are 
\first{\key{logical and}}{operator!and},
\first{\key{logical or}}{operator!or} and
\first{\key{logical not}}{operator!not}.
...
\index{built-in!panicln}
\todo{Needs to be written}
...
\gomarginpar{Go \gorelease{2010-10-27}} 
...
Go provides a syntax to take out of few elements from an array to create
a new slice, or to create a slice from a slice.
\begin{lstlisting}
// array[n:m], create a slice from element n to m
a := []int{1, 2, 3, 4, 5} |\longremark{Define an array with 5 elements;}|
s1 := a[2:4] |\longremark{Create a slice with the elements from index 2 to 4;}|
s2 := a[:]   |\longremark{Create a slice with all the elements of the %
array in it. This is a shorthand for: \texttt{a[0:len(a)]};}|
s3 := a[:4]  |\longremark{Create a slice with the element from index %
zero to index 4, this is thus short for: \texttt{a[0:4]};}|
s4 := s2[:]  |\longremark{Create a slice from a slice, note that %
\texttt{s4} still refers to the array \texttt{a}.}|
\end{lstlisting}
\showremarks
