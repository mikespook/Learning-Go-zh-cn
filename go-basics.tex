...
\begin{lstlisting}
s := "hello"
c := []int(s)	    |\coderemark{Convert s to an array.}|

c[0] = 'c'	    |\coderemark{Change the first element of this %
array.}|
s2 := string(c[0]) + string(c[1:]) |\coderemark{Create a \emph{new} %
...
\lstinline{var c complex = 5i-5i; fmt.Printf("Value is: %v", c")}

\subsection{Type conversion/Casting}
How to do type conversions, what is allowed and why \draft{TODO}
