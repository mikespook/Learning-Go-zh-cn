...
\lstinline{import} may come next. Always in Go, \lstinline{package} is first, then
...
\noindent\error{var.go:3: i declared and not used}
\begin{lstlisting}
package main
func main() { 
    var i int
...
(explicit) list for (signed and unsigned) integers is \lstinline{int8}, \lstinline{int16},
\lstinline{int32} and \lstinline{int64}. With \lstinline{byte} being an
...
Octal, hexadecimal and the scientific notation may be used: 077, 
0xFF, 1e3 or 6.022e23 are all valid.
...
\paragraph{\func{panic} and \func{panicln}} used for an elegant
