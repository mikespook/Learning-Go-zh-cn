...
slice as the key and value is set to the value of that key.
\todo{better}. Consider this code:
\begin{lstlisting}
list := []string{"a", "b", "c", "d", "e", "f"}|\longremark{Create a %
slice (see bllabla) of \type{string}s.}|
for k, v := range list {|\coderemark{Use \key{range} to loop over them.%
In each interation \key{range} will return the index as \type{int} and %
