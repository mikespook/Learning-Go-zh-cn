...
And then we run it:
\begin{display}
\pr ./6.out	    \coderemark{The default name for a (64 bit) Go executable (8.out on 32 bits)}
\end{display}
\vspace{-3.0ex}
...
wrapper around \prog{make}
\footnote{This is GNU Make.}. As the build system for Go programs may
...
s := `Starting part
     Ending part`
\end{lstlisting}
Be aware that in this example \var{s} now also contains the newline.
\end{lbar}
...
logical \first{\key{and}}{operator!and},
logical \first{\key{or}}{operator!or} and
logical \first{\key{not}}{operator!not}.
...
are not required and the bodie must \emph{always} be brace-delimited.
