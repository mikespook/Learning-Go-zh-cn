...
\verb|<number>l|. The go tool chain abstract this all away behind a tool
called \prog{go}\index{tooling!go!tool}. But you can still access the compiler
and linker directly.

To compile the code from above, we can use:
\begin{display}
\pr \user{go tool 6g helloworld.go}
\end{display}
\index{tooling!go|6g}

\noindent{}And then we link it with \prog{go tool 6l}:
\begin{display}
...
Another, less laborious and preferred, way to build a Go program, is to use a
the \prog{go} tool.\index{tooling!go}
The \prog{go} tools uses one configuration paramater \var{\$GOPATH}, that
points to a directory structure:
\begin{display}
\pr mkdir -p ~/g/src    # create the directories ~/g and ~/g/src
\pr GOPATH=~/g          # set GOPATH to ~/g
\end{display}

Create a holding directory for \prog{helloworld}:
\begin{display}
\pr mkdir ~/g/src/hello             # create hello subdirectory in src
\pr cp helloworld.go ~/g/src/hello  # copy the source code
\end{display}
...
This results in an executable called \prog{a.out}. If you want to use a better
name you can use the output (-o) flag:
\begin{display}
\pr go build -o helloworld
\end{display}
Resulting in a executable called \prog{helloworld}.
