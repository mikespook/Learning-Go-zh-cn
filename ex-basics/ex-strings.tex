...
\end{display}
同时输出这个字符串的字节数。
\emph{提示：} 看看 \package{utf8} 包。

\Question \label{ex:string q3} 扩展上一个问题的程序，替换位置 4 开始的三个字符为 'abc'。

\Question \label{ex:string q4} 编写一个 Go 程序可以逆转字符串，例如
"foobar" 被打印成 "raboof"。
\emph{提示：} 不幸的是你需要知道一些关于转换的内容，
去 "\titleref{sec:conversions}" 在 \pageref{sec:conversions}" 页的内容。
...
\Question 为了解决这个问题，需要 \package{utf8} 包的帮助。
首先，阅读一下文档 \prog{godoc utf8 | less}。在阅读文档的时候，会注意到
\lstinline{func RuneCount(p []byte) int}。然后，将
\emph{string} 转换为 \type{byte} slice：
\begin{lstlisting}
str := "hello"
b   := []byte(str) |\coderemark{转换，参阅 \pageref{sec:conversions} 页}|
\end{lstlisting}
