...
match the package name. Go packages may consist of multiple files,
...
variables) you define in a package.
...
\footnote{The descriptions are copied from the packages' \prog{godoc}.}

\begin{description}
\item[\package{fmt}]{\index{package!fmt}
Package \package{fmt} implements formatted I/O with functions analogous
to C's \func{printf} and \func{scanf}. The format \texttt{verbs} are derived
from C's but are simpler. Some verbs (\%-sequences) that can be used:
\begin{description}
\item[\lstinline{\%v}]{{The value in a default format.
when printing structs, the plus flag (\%+v) adds field names;}
\item[\lstinline{\%\#v]{a Go-syntax representation of the value.}
\item[\lstinline{\%T]{a Go-syntax representation of the type of the value;}
\end{description}
...
switch or type assertion can reveal which (See chapter \ref{chap:interfaces}, 
