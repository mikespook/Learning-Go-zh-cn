...
Now we can use the package in our own program \prog{myeven.go}:
...
10 to use the unexported function \func{even.odd}:
...
the accessor for the contents. After\index{package!bytes}
\begin{lstlisting}
import "bytes"
\end{lstlisting}
...
import bar "bytes"
\end{lstlisting}
The function \func{Buffer} is now accessed as \lstinline{bar.Buffer}. This
...
always addressed by their package name, \func{bufio.Reader} does not conflict
...
When writing test you will need to tell \prog{go test} whether a test was
