...
to put the package files in. TODO: setting GOPATH
\begin{display}
\pr mkdir even
\pr cp even.go even/
\end{display}
Once again (see "\titleref{sec:building a program}" in chapter \ref{chap:basics}) we can use 
the \prog{go} tool, but this time to build the package.
\begin{display}
\pr go build
\pr go install
\end{display}

\noindent{}After building and installing we can change our \key{import} from
...
The \prog{gotest} program runs all the test functions. Without any
defined tests for our \package{even} package, \prog{gotest} yields:
\begin{display}
\pr gotest
