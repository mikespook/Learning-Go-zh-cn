...
为了创建一个别人可以用的包（只需要 \lstinline{import "even"} 即可使用），首先需要创建一个目录放入包文件。参考 “\titleref{sec:settings used}”小节了解配置使用。
\begin{display}
\pr mkdir ~/g/src/even
\pr cp even.go ~/g/src/even/
\end{display}
同样的可以使用工具 \prog{go} （参考第\ref{chap:basics}章，“\titleref{sec:building a program}”小节），不过这次是用来编译包。
\begin{display}
\pr go build
\pr go install
\end{display}

\noindent{}在构建和安装之后，可以将 \key{import} 从
\lstinline{"./even"} 改为 \lstinline{"even"}。
...
这会出现在 \prog{go doc} 生成的关于包的页面上，并且相关的细节会跟随气候。
...
PASS
\end{display}
\noindent{}测试执行并且报告 \texttt{PASS}。成功了！
...
\package{json} 包实现了编码与解码 RFC 4627 \cite{RFC4627} 定义的 JSON 对象。
}
