...
time and them print something to the screen. A
\first{goroutine}{goroutine} is a normal function, except that you start
it with the keyword \first{\key{go}}{keyword!go}.
\begin{lstlisting}
ready("Tee", 2)	    |\coderemark{Normal function call}|
go ready("Tee", 2)  |\coderemark{\func{Tee()} started as goroutine}|
\end{lstlisting}
On the lines 14 and 15 we start two goroutines.
...
func main() {i
	...
	go ready("Tee", 2) |\longremark{Start the goroutines with the keyword \key{go}l;}|
	go ready("Coffee", 1)
