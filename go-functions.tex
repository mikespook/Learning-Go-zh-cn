...
\end{lstlisting}
On thing that is now allowed in Go is functions in functions. 
\begin{lstlisting}
func a() {
...
you are using named result parameters and use a function literal, i.e:
\begin{lstlisting}p[caption=Function literal]
defer func() {
	|\ldots|
}()
\end{lstlisting}
In that (unnamed) function you can then access any named return
parameter like so:
\begin{lstlisting}[caption=Access return values within \func{defer}]
func f() (ret int){
	defer func() {
		ret++	|\coderemark{Up \var{ret} with $1$}|
	}()
