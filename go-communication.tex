...
\lstinputlisting[numbers=right,caption=从文件读取（无缓冲）,label=src:read]{src/file.go}
如果想要使用\first{缓冲}{buffered} IO，则有
\package{bufio}\index{package!bufio} 包：
\lstinputlisting[numbers=right,caption=从文件读取（缓冲）,label=src:bufread]{src/buffile.go}
在第 9 行从 \type{*File} 类型的 \var{f} 中创建了 \type{bufio.Reader}。
\func{NewReader} 期望一个 \type{io.Reader}，所以或许认为这个会出错。
但是不会。\type{io.Reader} 定义为：
\begin{lstlisting}
type Reader interface {
    Read(p []byte) (n int, err os.Error)
}
\end{lstlisting}
因此，\emph{任何}有 \func{Read()} 函数的都实现了这个接口。
从表 \ref{src:read} （第 10 行）可以看到 \type{*File} 实际是这样做的。
