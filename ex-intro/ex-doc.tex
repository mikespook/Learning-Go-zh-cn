...
\end{display}
\vskip\baselineskip
哪个 \prog{godoc} 的命令可以显示 \package{compress} 包中的 \package{gzip} 文档？

\end{Exercise}

\begin{Answer}
\Question
\package{gzip} 包在 \package{compress} 的\emph{子目录}中，所以只需要 \quad \texttt{godoc
compress/gzip} 即可。
...
所有的内建函数同样可以通过 \prog{godoc builtin} 的方式在 godoc 中访问。
\end{Answer}
